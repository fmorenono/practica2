...
A coninuación se mostrarán los métodos ordenados descendentemente por el porcentaje de predicción de todos los modelos.

```{r}
vectorResultados = c(precisionC50ArbolClasificacion*100,precisionRegresionLogistica*100,precisionNaiveBayes*100)
M1 = cbind(vectorResultados,c("Árbol Clasificación", "Regresión Logística","Naive Bayes")) #Creamos la matriz 
colnames(M1) <- c("Porcentaje", "Modelo")
# Ordenamos y mostramos los resultados
M1[order(M1[,"Porcentaje"],decreasing = TRUE), ]
```


Los resultados del Cross Validation nos puede dar la visión por cuál de los modelos nos podemos inclinar, entregando un mejor resultado en la verisidad de la predicción. 
